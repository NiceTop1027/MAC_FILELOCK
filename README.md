...
태그 릴리스(`v1.0.5` 같은 형식)를 만들면 GitHub Actions가 자동으로 macOS 빌드와 DMG 아티팩트를 생성하도록 설정되어 있습니다.

Sparkle appcast 갱신은 로컬에서 아래처럼 수행합니다.

```bash
bash publish_update.sh 1.0.5 dist/FileLock.dmg
```
